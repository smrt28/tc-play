...
int tc_ykpiv_fetch_secret(int n, const char *pin, unsigned char *secret_out,
        char *errmsg);

int tc_ykpiv_init_and_verify(struct ykpiv_state **state,
        const char *pin, const char *mgm_key, char *errmsg);


int tc_ykpiv_select_slot(size_t n, uint8_t *key, int *object_id);
