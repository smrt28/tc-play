...
static int generate_key(int n, const char *mgm, const char *hexsecret) {
    ...
    rv = tc_ykpiv_init_and_verify(&state, NULL, mgm, errmsg);
    if (rv != 0) goto err;
...
static void usage() {
    printf("usage: tc-yubico-piv-tool -a action (setup|fetch) "
...
int main(int argc, char **argv) {
    ...
    if (!o.action) {
        CERROR(ERR_YK_ARGS, "yubico-tc-init: -a option required");
    }
    ...
    if (strcmp(o.action, "setup") == 0) {
        ...
        if ((rv = tc_ykpiv_fetch_secret(o.slot, o.pin, secret, errmsg)) == 0) {
